...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    if (request.action === 'unfollowUser') {
        handleUnfollowUser(request.userId, request.username, sendResponse);
        return true;
    }
});

async function handleUnfollowUser(userId, username, sendResponse) {
    try {
        console.log(`Attempting to unfollow ${username} (ID: ${userId})...`);
        ...
        if (response.ok) {
            ...
            if (data.status === 'ok') {
                ...
                sendResponse({ success: false, error: 'Instagram returned an error' });
            }
        } else {
...
async function fetchFollowers(userId) {
    try {
        ...
        while (hasNext) {
            ...
            console.log(`Followers: fetched ${followers.length} so far... (page ${count}), hasNext: ${hasNext}`);
...
async function fetchFollowing(userId) {
    try {
        ...
        while (hasNext) {
            ...
            console.log(`Following: fetched ${following.length} so far... (page ${count}), hasNext: ${hasNext}`);

            // Small delay to avoid rate limiting
            if (hasNext) await sleep(1500);
        }

        console.log(`Fetched ${following.length} following total`);
        return following;
